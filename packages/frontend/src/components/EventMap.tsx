...
export default function EventMap({ coordinates, address, eventTitle, className = "" }: EventMapProps) {
  ...
  useEffect(() => {
    addDebugInfo(`Component mounted with coordinates: ${coordinates}, address: ${address}`);
    ...
    const initializeMap = async () => {
      try {
        ...
        if (coordinates) {
          ...
          const [latStr, lngStr] = coordinates.split(',').map(coord => coord.trim());
          ...
          const infoWindow = new (window.google as any).maps.InfoWindow({
            content: `
              <div style="padding: 8px;">
                <h3 style="margin: 0 0 4px 0; font-size: 14px;">${eventTitle || 'Event Location'}</h3>
                ${address ? `<p style="margin: 0; font-size: 12px; color: #666;">${address}</p>` : ''}
                <p style="margin: 4px 0 0 0; font-size: 12px; color: #999;">${lat.toFixed(6)}, ${lng.toFixed(6)}</p>
              </div>
            `
          });
  ...
  return (
    <div className={`relative ${className}`}>
       <a className="button-blue mb-10" href={`https://www.google.com/maps/search/?api=1&query=${coordinates}`} target="_blank" rel="noopener noreferrer">
        Open in Google Maps
      </a>
      <div 
        ref={mapRef} 
