 ...
 * Usage: tsx src/check-production-function-logs.ts [--days 1]
 */
...
async function checkProductionFunctionLogs(days: number = 1) {
  // Based on Step Functions check, the production function is:
  const logGroupName =
    "/aws/lambda/touchgrassdcsst-production-addEventToDBFunctionFunction-cvvsmmbs";
  const startTime = Date.now() - days * 24 * 60 * 60 * 1000;

  console.log(`🔍 Checking production function logs...\n`);
  ...
  try {
    ...
    // Get all logs including debug info
...
async function main() {
  ...
  await checkProductionFunctionLogs(days);
}
