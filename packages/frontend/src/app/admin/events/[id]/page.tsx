...
export default function AdminEventDetailPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      alert(`Failed to approve event: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Submitted by {event.email}</p>
              </div>
          ...
          <div className="px-6 py-6 space-y-6">
                ...
                <dl className="space-y-2">
                  ...
                  {event.eventDate && (
                    <div>
                      ...
                      <dd className="text-gray-900">{new Date(event.eventDate).toLocaleDateString()}</dd>
                    </div>
                  )}
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Submitted</dt>
                    <dd className="text-gray-900">{new Date(parseInt(event.createdAt)).toLocaleDateString()}</dd>
                  </div>
                </dl>
              </div>
            ...
            {event.image_url && (
              <div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">Event Image</h3>
                <img 
                  src={event.image_url} 
                  alt={event.title}
