...
export default function HomepageMap({ className = "" }: HomepageMapProps) {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch('/api/events?fields=title,description,location,coordinates,cost,category,date&is_public=true');
        ...
        const eventsWithLocation = data.events.filter((event: Event) => 
      ...
      try {
        ...
        events.forEach(event => {
          if (event.coordinates) {
            const [latStr, lngStr] = event.coordinates.split(',').map(coord => coord.trim());
        ...
        const center = hasValidCoordinates ? bounds.getCenter() : { lat: 38.9072, lng: -77.0369 };
              ...
              const infoWindow = new (window.google as any).maps.InfoWindow({
                content: `
                  <div style="padding: 12px; max-width: 250px; font-family: system-ui, -apple-system, sans-serif;">
                    ...
                    ${event.description ? `<p style="margin: 0 0 8px 0; font-size: 14px; color: #6B7280; line-height: 1.4;">${event.description}</p>` : ''}
                    ${event.location ? `<p style="margin: 0 0 6px 0; font-size: 13px; color: #374151;"><strong>📍</strong> ${event.location}</p>` : ''}
                    ${event.date ? `<p style="margin: 0 0 6px 0; font-size: 13px; color: #374151;"><strong>📅</strong> ${event.date}</p>` : ''}
                    ${event.category ? `<p style="margin: 0 0 6px 0; font-size: 13px; color: #374151;"><strong>🏷️</strong> ${event.category}</p>` : ''}
                    ${event.cost ? `<p style="margin: 0; font-size: 13px; color: #374151;"><strong>💰</strong> ${typeof event.cost === 'object' ? event.cost.amount : event.cost}</p>` : ''}
                  </div>
                `,
                maxWidth: 300,
              });
  ...
  if (error) {
    return (
      <div className={`${className} flex items-center justify-center h-96 bg-red-50 rounded-lg`}>
        <div className="text-center">
