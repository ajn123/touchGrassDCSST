...
export function decodeHtmlEntities(input?: string): string {
  ...
  out = out.replace(/&#[xX]([0-9a-fA-F]+);/g, (_m, n) =>
...
export function generateStyledEventSvgBuffer({
  ...
  const titleSvg = titleLines
    .map(
      (line, i) =>
        `<text x="600" y="${titleStartY + i * lineHeight}" fill="#f8fafc" font-family="system-ui,-apple-system,sans-serif" font-size="52" font-weight="700" letter-spacing="-0.5" text-anchor="middle">${line}</text>`
    )
    .join("\n  ");
 ...
 */
export function generateStyledArticleSvgBuffer({
  title,
  category,
  subtitle,
}: {
  ...
  const titleSvg = titleLines
    .map(
      (line, i) =>
        `<text x="600" y="${titleStartY + i * lineHeight}" fill="#f8fafc" font-family="system-ui,-apple-system,sans-serif" font-size="64" font-weight="700" letter-spacing="-0.5" text-anchor="middle">${line}</text>`
    )
    .join("\n  ");
