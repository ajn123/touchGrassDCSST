...
export async function POST(request: NextRequest) {
  try {
    console.log("🚀 Manual DC Improv crawler trigger requested");
    // By default, 'await task.run(Resource.dcimprovTask)' only waits for the ECS task to be started (RUNNING),
    ...
    //
    // Example:
    const runRet = await task.run(Resource.dcimprovTask);

    return NextResponse.json({ ...runRet });
  } catch (error) {
    console.error("❌ Failed to trigger DC Improv crawler:", error);
